...
para_gk = ['clean_sheets','saves','penalties_saved',"games_starts_gk",'goals_conceded']
val_gk = [best_gk['clean_sheets'].mean()*4,best_gk['saves'].mean()*1,
        best_gk['penalties_saved'].mean()*5,best_gk['games_starts_gk'].mean()*2,
            best_gk['goals_conceded'].mean()*1]

para_df = ['clean_sheets','goals_scored','assists','yellow_cards','games_starts','goals_conceded']
val_df = [best_df['clean_sheets'].mean()*4,best_df['goals_scored'].mean()*6,
        best_df['assists'].mean()*3,best_df['yellow_cards'].mean()*1,
        best_df['games_starts'].mean()*2,best_df['goals_conceded'].mean()*1]

para_mf = ['clean_sheets','goals_scored','assists','yellow_cards','games_starts']
val_mf = [best_mf['clean_sheets'].mean()*1,best_mf['goals_scored'].mean()*5,
        best_mf['assists'].mean()*3,best_mf['yellow_cards'].mean()*1,
        best_mf['games_starts'].mean()*2]
...
fig_gk = px.pie(values=val_gk,names=para_gk,title='Goal Keepers parameters',labels=para_gk
        ,color=para_gk
        ,color_discrete_map={"clean_sheets":'darkblue','penalties_saved':'goldenrod'
         ,'saves':'1f77b24','games_starts_gk':'109618','goals_conceded':'d62728'})
...
fig_df = px.pie(values=val_df,names=para_df,title='Defenders parameters',labels=para_df
        ,color=para_df
       ,color_discrete_map={"clean_sheets":'1f77b24','goals_scored':'goldenrod',
            'assists':'ff7f0e','yellow_cards':'yellow','games_starts':'109618'
            ,'goals_conceded':'d62728'})
...
fig_fw = px.pie(values=val_fw,names=para_fw,title='Forwards parameters',labels=para_fw
        ,color=para_fw
       ,color_discrete_map={'goals_scored':'goldenrod',
            'assists':'ff7f0e','yellow_cards':'yellow','games_starts':'109618'})
