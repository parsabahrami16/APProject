...
del_null_pl(pl_time)
global players
players=del_null_pl(elements_df)
...
def add_full_name(df):#add full name to players
    ...
    for row in df.index:
        ...
        if first_name in web_name: #check if web_name is enough
            name=web_name
        else:
        ...
        for row in fantasy.index: #iterate on players of a specific team
            ...
            for s_row in fbref.index:
                ...
                if name==fb_name or first_name+' '+second_name==fb_name: #find names which are totally equal
                    ...
                    fantasy.drop(index=row,inplace=True)#remove checked player
            ...
            if checked==False: #find names which are not totally equal
                for s_row in fbref.index:
                    ...
                    fb_team=fbref['squad'][s_row]
                    fb_pos=fbref['position'][s_row]
                    if web_name in fb_name or second_name in fb_name:#check other conditions
                        df.loc[df['player']==fb_name,'id']=player_id
                        checked=True
                    elif (first_name in fb_name or fb_name in first_name) and pos in fb_pos:
                        df.loc[df['player']==fb_name,'id']=player_id
                        checked=True
            if checked==False:#for winter transfers!
...
pd.options.mode.chained_assignment = None 
def add_cl_fantasy(post,df,cl): #add columns from teams_df to players
    post[cl] = 0 
...
for stg in stg_list: #adding strength defence & attack to DF & MF
    add_cl_fantasy(DF,teams_df,stg)
    add_cl_fantasy(MF,teams_df,stg)
add_cl_fantasy(FW,teams_df,stg_list[0])#add to GK
add_cl_fantasy(FW,teams_df,stg_list[1])
add_cl_fantasy(GK,teams_df,stg_list[2])#add to FW
add_cl_fantasy(GK,teams_df,stg_list[3])
...
def add_cl_fbref(post,fbref,cl_list): #add columns from fbref to fantasy
    ...
    for row in post.index:
        for cl in cl_list:
            ...
            fb_row.index=[0]
            post[cl][row]=fb_row[cl][0]#set data
